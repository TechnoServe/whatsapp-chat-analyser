function Analyser() {
  ...
  this.edit_template =
    "<button class='%s btn btn-outline-info btn-sm' data-toggle='modal' data-target='%s' data-object_type='%s' data-row-id='%s' data-action='edit'>Edit</button>";
  this.delete_template =
    "<button class='%s btn btn-outline-danger btn-sm' data-object_type='%s' data-action='delete' data-row-id='%s' data-toggle='modal' data-target='#confirmModal' >Delete</button>";
  this.activate_template =
    "<button class='%s btn btn-outline-%s btn-sm' data-object_type='%s' data-row-id='%s' data-toggle='modal' data-action='%s' data-target='#confirmModal'>%s</button>";
...
Analyser.prototype.initiatePages = function () {
  ...
  else if (window.location.pathname == "/whatsapp_groups_bc")
    this.initiateGroups();
...
Analyser.prototype.initiateSysUsers = function (event) {
  ...
  // TODO (Done) on user edit assign values
...
Analyser.prototype.drawGroupStatsGraphs = function () {
  ...
  Highcharts.chart("active_days", {
    ...
    xAxis: {
      categories:
        (analyser.stats.active_dates && analyser.stats.active_dates.dates) ||
        "",
      crosshair: true,
    },
    credits: analyser.graph_credits,
    ...
    series: [
      {
        ...
        color: "#d1e97b",
        data:
          (analyser.stats.active_dates &&
            analyser.stats.active_dates.messages) ||
          [],
      },
      {
        name: "Active Users",
        ...
        color: "#00b0af",
        data:
          (analyser.stats.active_dates && analyser.stats.active_dates.users) ||
          [],
      },
    ],
  });
