/**
 * When the download button is clicked we grab the file id and do the JSON
 */
function clickedResend(e) {
  const clicked_btn = $(e.target);
  let fileId = clicked_btn.attr("attr-fileId");
  clicked_btn[0].innerText = "...sending";
  $.ajax({
    type: "GET",
    url: `/resend_report?fileId=${fileId}`,
    dataType: "json",
    success: function (data) {
      clicked_btn[0].innerText = "Resend Report";
    },
    error: function (data) {
      clicked_btn[0].innerText = "Resend Report";
    },
...
function Analyser() {
  ...
  this.edit_template =
    "<button title='Edit' class='%s btn btn-info btn-sm' data-toggle='modal' data-target='%s' data-object_type='%s' data-row-id='%s' data-action='edit'><i class='fa fa-edit'></i></button>";
  this.delete_template =
    "<button title='Delete' class='%s btn btn-danger btn-sm' data-object_type='%s' data-action='delete' data-row-id='%s' data-toggle='modal' data-target='#confirmModal' ><i class='fa fa-trash-o'></i></button>";
  this.activate_template =
    "<button title='Deactivate' class='%s btn btn-%s btn-sm' data-object_type='%s' data-row-id='%s' data-toggle='modal' data-action='%s' data-target='#confirmModal'><i class='fa fa-power-off'></i></button>";

  this.resend_link =
    "<button title='Resend Link' class='%s btn btn-success btn-sm' data-row-id='%s' ><i class='fa fa-repeat'></i></button>";

  this.download_template =
    "<a href='/download_file?filename=%s'  title='Download' class='%s btn btn-info btn-sm' data-target='%s' data-object_type='%s' data-row-id='%s' data-action='download'>Download</a>";
  (this.download_disabled =
    "<a href='/download_file?filename=%s'  title='Scan before download' class='%s btn btn-dark btn-sm disabled' data-target='%s' data-object_type='%s' data-row-id='%s' data-action='scan'>Process</a>"),
    (this.resend_template = `<button onclick="clickedResend(event)" attr-fileId='%s'  title='Download' class='%s btn btn-info btn-sm resend-btn' data-target='%s' data-object_type='%s' data-row-id='%s' data-action='resend'>Resend Report</button>`);
  (this.resend_disabled = ""),
...
Analyser.prototype.initiatePages = function () {
  if (window.location.pathname == "/dashboard") this.initiateDashboard();
  else if (window.location.pathname == "/groups") this.initiateGroups();
  else if (window.location.pathname == "/engaged_users")
    this.initiateEngagedUsers();
  else if (window.location.pathname.split("/")[1] == "group_stats")
    this.initiateGroupStats();
  else if (window.location.pathname.split("/")[1] == "user_stats")
    this.initiateUserStats();
...
Analyser.prototype.initiateSysUsers = function (event) {
  analyser.button_settings["user"]["table"] = $("#sys_users").DataTable({
    ...
    columnDefs: [
      ...
      },
      {
        ...
        render: function (data, type, row, meta) {
          // Cache the user
          localStorage.setItem(row.pk_id, JSON.stringify(row));

          edit_btn = sprintf(
            analyser.edit_template,
            "edit_user",
            "#editUser",
            "user",
            row.pk_id
  ...
  // TODO (Done) on user edit assign values when modal opened
  $("#editUser").on("show.bs.modal", function (event) {
    if (analyser.edituser_validator != undefined) {
      analyser.edituser_validator.resetForm();
      $(".form-control.error").removeClass("error");
    }
    let button = event.relatedTarget;
    let userId = $(button).data("row-id");
    const user = JSON.parse(localStorage.getItem(userId));
    let form = $('form[name="edit_user"]')[0];
    form["edit_user_id"].value = user.pk_id;
    form["edit_first_name"].value = user.first_name;
    form["edit_surname"].value = user.last_name;
    form["edit_username"].value = user.username;
    form["edit_email"].value = user.email;
    form["edit_tel"].value = user.tel;
    form["edit_designation"].value = user.designation;

    $("#confirm_edit_save").off(); // remove previous event listeners
    $("#confirm_edit_save").on("click", () => {
      let formSubmit = $("[name=edit_user]");
      analyser.edituser_validator = formSubmit.validate({
        debug: true,
        rules: {
          email: {
            email: true,
            remote:
              user.email == form["edit_email"].value
                ? false
                : {
                    url: "validate_input",
                    async: false, // find a way to remove this
          ...
          username: {
            minlength: 3,
            remote:
              user.username == form["edit_username"].value
                ? false
                : {
                    url: "validate_input",
                    async: false, // find a way to remove this
      ...
      let isValid = formSubmit.valid();
      if (isValid) {
        let ajax_data = objectifyForm($("[name=edit_user]").serializeArray());
...
Analyser.prototype.initiateObjectManagement = function () {
  $("#confirmModal").on("show.bs.modal", function (event) {
    ...
    if (analyser.button_settings[analyser.cur_object]["is_long_process"]) {
      modal_message +=
        "<br /><br />This will take a few seconds as we save the changes and update the data collection app...";
    }
  ...
  $("#confirm").on("click", function () {
    ...
    var url =
      "/" +
      analyser.cur_action +
      analyser.button_settings[analyser.cur_object]["url"];
    analyser.refresh_table =
      analyser.button_settings[analyser.cur_object]["table"];
    analyser.showProcessing();
    $.ajax({
      type: "POST",
      url: url,
      dataType: "json",
      data: analyser.ajax_data,
      error: analyser.communicationError,
      success: function (data) {
        analyser.endShowProcessing();
        if (data.error) {
          ...
          var message =
            data.message == undefined
              ? analyser.button_settings[analyser.cur_object]["success_message"]
              : data.message;
          ...
          $(
            "[name=" +
              analyser.button_settings[analyser.cur_object]["form"] +
              "]"
          ).trigger("reset");
...
Analyser.prototype.initiateGroups = function () {
  ...
  var table = $("#whatsapp_groups").DataTable({
    ...
    columns: [
      ...
      // },
      {
        ...
        render: function (data, type, row, meta) {
          return (
            "<a href='#' class='group_link' data-group_id='" +
            row.group_id +
            "'>" +
            row.group_name +
            "</a>"
      ...
      { data: "counselor_", targets: 9 },
      {
        ...
        render: function (data, type, row, meta) {
          return (
            "<a href='#' class='assign_group_counselor' data-group_id='" +
            row.group_id +
            "'>" +
            "Assign Counselor" +
            "</a>"
  ...
  $(document).on("click", ".group_link", function () {
    analyser.showItemStats(
      sprintf("/group_stats/%s", $(this).data("group_id"))
    );
  });
};
  ...
  var table = $("#engaged_users").DataTable({
    ...
    columnDefs: [
      ...
      },
      {
        ...
        render: function (data, type, row, meta) {
          return (
            "<a href='#' class='user_link' data-group_id='" +
            row.group_id +
            "' data-user_id='" +
            row.name_phone +
            "'>" +
  ...
  analyser.files_table = $("#exported_chats").DataTable({
    ...
    columnDefs: [
      ...
      { data: "email", targets: 6, sWidth: "25%" },
      {
        ...
        targets: 7,
        render: function (data, type, row, meta) {
          download_disabled_btn = sprintf(
            analyser.download_disabled,
            "reprocess_file",
            "#reprocessFile",
            "reprocess_file",
            row.pk_id
          ...
          if (row.status == "processed") return download_btn;
      ...
      },
      {
        ...
        targets: 8,
        render: function (data, type, row, meta) {
          resend_disabled_btn = sprintf(
            analyser.resend_disabled,
            "reprocess_file",
            "#reprocessFile",
            "reprocess_file",
            row.pk_id
...
Analyser.prototype.initiateDashboard = function () {
  ...
  var gauge1 = Gauge(document.getElementById("processing_progress"), {
    max: 100,
    dialStartAngle: -90,
    dialEndAngle: -90.001,
    value: 100,
...
Analyser.prototype.initiateDateRanges = function () {
  if (analyser.stats == undefined || analyser.stats.max_date == undefined) {
    ...
    my_max_date = moment(analyser.stats.max_date, "DD/MM/YYYY").format(
      "DD/MM/YYYY"
    );
    my_min_date = moment(analyser.stats.min_date, "DD/MM/YYYY").format(
      "DD/MM/YYYY"
    );
  }
  ...
  if (analyser.stats == undefined || analyser.stats.s_date == undefined) {
    picker_settings["startDate"] = moment()
      .subtract(90, "days")
      .format("DD/MM/YYYY");
    picker_settings["endDate"] = moment()
      .subtract(60, "days")
      .format("DD/MM/YYYY");
  } else {
    picker_settings["startDate"] = moment(
      analyser.stats.s_date,
      "DD/MM/YYYY"
    ).format("DD/MM/YYYY");
    picker_settings["endDate"] = moment(
      analyser.stats.e_date,
      "DD/MM/YYYY"
    ).format("DD/MM/YYYY");
  }

  $('input[name="analysis_range"]').daterangepicker(picker_settings);
  $('input[name="analysis_range"]').on(
    "apply.daterangepicker",
    function (ev, picker) {
      analyser.showItemStats(window.location.pathname);
    }
  );
};
...
Analyser.prototype.drawGroupStatsGraphs = function () {
        ...
        dataLabels: {
          ...
          formatter: function () {
            return sprintf(
              "%s: %.1f%%",
              this.point.name,
              (this.point.y / analyser.stats.totals) * 100
            );
          },
        },
      },
  ...
  var gauge1 = Gauge(document.getElementById("attrition"), {
    max: 100,
    dialStartAngle: -90,
    dialEndAngle: -90.001,
    value: 100,
  ...
  gauge1.setValueAnimated(
    ((analyser.stats.lefties / analyser.stats.no_users) * 100).toFixed(1),
    1
  );

  var gauge2 = Gauge(document.getElementById("joining"), {
    max: 100,
    dialStartAngle: -90,
    dialEndAngle: -90.001,
    value: 100,
  ...
  gauge2.setValueAnimated(
    ((analyser.stats.joinies / analyser.stats.no_users) * 100).toFixed(1),
    1
  );
};
...
Analyser.prototype.initiateSearchAutocomplete = function () {
  $("#global_search").autocomplete({
    ...
    onSelect: function (selection) {
      ...
      if (selection.data.category == "Group")
        url = sprintf("/group_stats/%s", selection.data.id);
      else if (selection.data.category == "User")
        url = sprintf(
          "/user_stats/%s/%s",
          selection.data.group_id,
          selection.data.user
        );
      else if (selection.data.category == "File")
        url = sprintf("/file_stats/%s", selection.data.id);
...
Analyser.prototype.drawUserStatsGraphs = function () {
  ...
  var gauge1 = Gauge(document.getElementById("group_interaction"), {
    max: 100,
    dialStartAngle: -90,
    dialEndAngle: -90.001,
    value: 100,
...
function populateChatDiv(data) {
  $("#chat_modal").empty();

  data.forEach((chat) => {
    let msg = chat.message;

    if (!msg.includes("omitted")) {
      row = '<li class="chat_item darker">';
      row += '<div class="row">';
      row += '<div class="col-lg-8">';
      row += "<p>" + chat.message + "</p>";
      row += '<span class="time-left">' + chat.datetime_sent + "</span>";
      row += "</div>";
      row += '<div class="col-lg-8">';
      row += '<span style="float: right;">' + chat.user.name_phone + "</span>";
      row += "</div>";
      row += "</div>";
      row += "</div>";

      $("#chat_modal").append(row);
    }
  });
}
