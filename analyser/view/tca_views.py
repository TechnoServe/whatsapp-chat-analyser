...
def validate_user_access(request, data_type):
    ...
    # data_collector
    access_rights = {"users": ["admins"]}
    try:
        if (
            request.session["cu_issuperuser"]
            or request.session["cu_designation_id"] == "system_admin"
            or request.session["cu_designation_id"] == "business_counselor"
            or request.session["cu_designation_id"] == "business_advisor"
            or request.session["cu_designation_id"] == "program_manager"
        ):
            return True
        if request.session["cu_designation_id"] in access_rights[data_type]:
            return True
...
def login_page(request, *args, **kwargs):
    csrf_token = get_or_create_csrf_token(request)
    page_settings = {
        "page_title": "%s | Login Page" % settings.SITE_NAME,
    ...
    try:
        ...
        if username is not None:
            ...
            if user is None:
                terminal.tprint(
                    "Couldn't authenticate the user... redirect to login page", "fail"
                )
                page_settings["error"] = (
                    settings.SITE_NAME
                    + " could not authenticate you. You entered an invalid username or password"
                )
        ...
        page_settings["message"] = (
            page_settings["message"]
            if "message" in page_settings
            else "Please enter your username and password"
        )
        return render(request, "signin.html", page_settings)
        ...
        page_settings[
            "message"
        ] = "There was an error while authenticating you. Please try again and if the error persist, please contact the system administrator"
        return render(request, "signin.html", page_settings)
...
def update_password(uid, password, token):
    try:
        ...
        email_settings = {
            ...
            "message": "Dear %s,<br /><p>You have successfully updated your password to the %s. You can now log in using your new password.</p>"
            % (user.first_name, settings.SITE_NAME),
        }
        notify = Notification()
...
def activate_user(request, user, token):
    try:
        ...
        email_settings = {
            ...
            "message": "Thank you for confirming your email. Your account at %s is now active."
            % settings.SITE_NAME,
        }
        notify = Notification()
        notify.send_email(email_settings)
        ...
        sentry_sdk.capture_exception(e)
        return reverse(
            "home", kwargs={"error": True, "message": "The specified user doesnt exist"}
        )
        ...
        return reverse(
            ...
            kwargs={
                "error": True,
                "message": "There was an error while activating your account. Contact the system administrator",
            },
        )
    ...
    try:
        if uid:
            ...
            email_settings = {
                ...
                "use_queue": getattr(settings, "QUEUE_EMAILS", False),
                "verification_link": "http://%s/new_user_password/%s/%s"
                % (current_site.domain, uid, token),
                "message": "Someone, hopefully you tried to reset their password on %s. Please click on the link below to reset your password."
                % settings.SITE_NAME,
        ...
        return render(
            ...
            "signin.html",
            {
                ...
                "is_error": True,
                "message": "There was an error while saving the new password",
            },
        )
    ...
    try:
        ...
        if passwd != repeat_passwd:
            params = {
                ...
                "recover_user": uid,
                "is_error": True,
                "message": "The entered passwords dont match. Please try again.",
            }
            return render(request, "signin.html", params)
        ...
        return login_page(
            request, kwargs={"user": {"pass": passwd, "username": u_user.username}}
        )
    except Exception as e:
        if settings.DEBUG:
            print(str(e))
...
def get_ajax_data(request, d_type, filter_=None):
    try:
        ...
        if d_type == "users":
            ...
            data_to_return = PersonnelSerializer(
            ...
            all_groups = WhatsAppGroup.objects.filter(
                group_name__icontains=search_query
            ).all()
            all_groups_ser = WhatsAppGroupSerializer(
                all_groups, many=True, context={"request": request}
            )
            for grp in all_groups_ser.data:
                all_suggestions.append(
                    {
                        "value": grp["group_name"],
                        "data": {"category": "Group", "id": grp["pk_id"]},
                    }
                )

            all_users = (
                UserDailyStats.objects.select_related("group")
                .filter(name_phone__icontains=search_query)
                .values("name_phone", "group__group_name", "group_id")
                .annotate(
                    a=Count("name_phone", distinct=True),
                    b=Count("group_id", distinct=True),
                )
                .all()
            )
            for user in all_users:
                all_suggestions.append(
                    {
                        "value": "%s - %s"
                        % (user["name_phone"], user["group__group_name"]),
                        "data": {
                            "category": "User",
                            "user": user["name_phone"],
                            "group_id": my_hashids.encode(user["group_id"]),
                        },
                    }
                )

            to_return = {"query": search_query, "suggestions": all_suggestions}
        ...
        return JsonResponse(
            {
                "error": "There was an error while fetching data from the server. Please contact the system administrator."
            },
            status=500,
            safe=False,
        )
...
def dashboard(request, *args, **kwargs):
    try:
        ...
        return index(
            request,
            error=True,
            message="There was an error while getting a list of the WhatsApp groups. Please contact the system administrator",
        )
...
@user_passes_test(
    ...
    or u.designation == "data_manager"
    or u.designation == "system_admin",
    login_url="/dashboard",
)
...
def list_whatsapp_users(request):
    try:
        ...
        return dashboard(
            request,
            error=True,
            message="There was an error while getting a list of enagaged users. Please contact the system administrator",
        )
...
def list_exported_files(request):
    try:
        ...
        return dashboard(
            request,
            error=True,
            message="There was an error while getting a list of the exported files. Please contact the system administrator",
        )
...
@user_passes_test(
    ...
    or u.designation == "data_manager"
    or u.designation == "system_admin"
    or u.designation == "business_counselor"
    or u.designation == "business_advisor"
    or u.designation == "program_manager",
    login_url="/dashboard",
)
def show_group_stats(request, uid):
    try:
        ...
        return dashboard(
            request,
            error=True,
            message="There was an error while generating the analysis for the selected group. Please contact the system administrator",
        )
...
def show_user_stats(request, gid, user_):
    try:
        ...
        sentry_sdk.capture_exception(e)
        mssg = (
            "There was an error while generating the analysis for the selected user - %s"
            % user_
        )
        return dashboard(
            request,
            error=True,
            message="%s. Please contact the system administrator" % mssg,
        )
...
@login_required(login_url="/login")
@user_passes_test(
    lambda u: u.is_superuser or u.designation == "system_admin", login_url="/dashboard"
)
def users(request):
    params = get_basic_info(request)
...
def validate_objects(request):
    for key, value in request.GET.items():
        if key == "tel":
            ...
            to_return = (
                "true"
                if Personnel.objects.filter(tel=value).first() is None
                else "false"
            )
        elif key == "email":
            # check that this telephone is not already used
            to_return = (
                "true"
                if Personnel.objects.filter(email=value).first() is None
                else "false"
            )
        elif key == "username":
            # check that this telephone is not already used
            to_return = (
                "true"
                if Personnel.objects.filter(username=value).first() is None
                else "false"
            )
...
def add_objects(request, d_type):
    ...
    try:
        if d_type == "add_user":
            ...
            if (
                request.session["cu_issuperuser"]
                or request.session["designation"] == "system_admin"
            ):
        ...
        return JsonResponse(
            {
                "error": True,
                "message": "Please check the number of items being issued/received",
            }
        )
        ...
        sentry_sdk.capture_exception(e)
        return JsonResponse(
            {"error": True, "message": "There was an error while updating the database"}
        )
...
def add_user(request):
    ...
    try:
        ...
        email_settings = {
            ...
            "use_queue": getattr(settings, "QUEUE_EMAILS", False),
            "verification_link": "http://%s/activate_new_user/%s/%s"
            % (current_site.domain, uid, activation_link),
            "message": "You have been registered successfully to the %s. We are glad to have you on board. Please click on the button below to activate your account. You will not be able to use your account until it is activated. The activation link will expire in %d hours"
        ...
        return {
            "error": True,
            "message": "There was an error while saving the user: %s" % str(e),
        }
    except Exception as e:
        if settings.DEBUG:
            terminal.tprint(str(e), "fail")
...
def edit_objects(request, d_type):
    ...
    if (
        ...
        try:
            ...
            sentry_sdk.capture_exception(e)
            return JsonResponse(
                {"error": True, "message": "Please check the entered data"}
            )
            ...
            return JsonResponse(
                {
                    "error": True,
                    "message": "There was an error while updating the database",
                }
            )
    else:
        return JsonResponse(
...
def delete_objects(request, d_type):
    ...
    try:
        ...
        if d_type == "delete_user":
            ...
            user.delete()
            return JsonResponse(
                {"error": False, "message": "The user has been deleted successfully"}
            )
    except Exception as e:
        if settings.DEBUG:
            terminal.tprint(str(e), "fail")
...
def resend_activation_email(request):
    ...
    try:
        ...
        return JsonResponse(
            {"error": False, "message": "Activation email successfully sent"}
        )
    ...
    try:
        ...
        if re.search("^delete", current_path) and u.designation == "system_admin":
            ...
            return HttpResponse(
                json.dumps(
                    {
                        "error": False,
                        "message": "The %s has been deleted successfully"
                        % re.search("_(.+)$", current_path).group(1),
                    }
                )
            )

        elif re.search("^deactivate", current_path) and (
            u.designation == "system_admin" or u.is_superuser
        ):
            ...
            to_return = json.dumps(
                {
                    "error": False,
                    "message": "The %s has been updated successfully"
                    % re.search("_(.+)$", current_path).group(1),
                }
            )

        elif re.search("^activate", current_path) and (
            u.designation == "system_admin" or u.is_superuser
        ):
            cur_object.is_active = not cur_object.is_active
            cur_object.save()
        ...
        return HttpResponse(
            json.dumps(
                {
                    "error": True,
                    "message": "There was an error while managing the %s"
                    % re.search("_(.+)$", current_path).group(1),
                }
            )
        )
    ...
    try:
        ...
        return HttpResponse(
            json.dumps(
                {
                    "error": True,
                    "message": "There was an error while processing the new files. Check your email.",
                }
            )
        )
